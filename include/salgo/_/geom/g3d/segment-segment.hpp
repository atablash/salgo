...
auto orient_segments(const SEG& a, const SEG& b) {
    using Vector = std::remove_const_t<
		std::remove_reference_t< decltype(a.vert(0).pos()) >
	>;
	...
	auto A = (m * a.vert(0).pos()).eval();
	auto B = (m * a.vert(1).pos()).eval();
	auto C = (m * b.vert(0).pos()).eval();
	auto D = (m * b.vert(1).pos()).eval();
	auto BA = (B - A).eval();
	auto DC = (D - C).eval();
	auto CA = (C - A).eval();
	auto BC = (B - C).eval();
	...
	Vector p0 = a.vert(0).pos() + BA3 * alpha;
	Vector p1 = b.vert(0).pos() + DC3 * beta;
