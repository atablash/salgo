...
namespace salgo::_::chunked_array {
...
using H0 = Int_Handle<int,31>;

template<class X>
struct Handle : Pair_Handle_Base<Handle<X>, H0, int> { // for first number, char is enough
	using BASE = Pair_Handle_Base<Handle<X>, H0, int>;
	...
	Handle(const Handle_Small<X>& small) {
		...
		BASE::a = H0( bit_scan_reverse(small.a) );
...
struct Context {
	...
	using       Handle = chunked_array::      Handle<P>;
	using Handle_Small = chunked_array::Handle_Small<P>;
	...
	class Chunked_Array : private Add_num_existing<Count> {
	public:
	...
	private:
		...
		//
	private:
		salgo::Dynamic_Array< Memory_Block > _blocks;
		int _size = 0;
	...
	public:
		Chunked_Array() {
			static_assert(!(Dense && Count), "no need for COUNT if array is DENSE");
		}

		Chunked_Array(int new_size) : Chunked_Array() { resize(new_size); }

		template<class T>
		Chunked_Array(std::initializer_list<T>&& l) : Chunked_Array() {
		...
		Chunked_Array(const Chunked_Array&) = default;
		Chunked_Array(Chunked_Array&&) = default;

		Chunked_Array& operator=(const Chunked_Array&) = default;
		Chunked_Array& operator=(Chunked_Array&&) = default;
	...
	public:
		...
		Accessor<MUTAB> emplace_back(ARGS&&... args) {
			...
			return Accessor<MUTAB>( this, Handle(H0(_blocks.size()-1), idx) );
		}
	...
	private:
		...
		std::conditional_t<std::is_move_constructible_v<V>, V, void> _pop_back() {
			...
			DCHECK_GT(_size, 0) << "pop_back() on empty Chunked_Array";
	...
	public:
		int size() const {
			static_assert(Dense, "size() for sparse array is a bit ambiguous. Use count() or domain() instead");
			return _size;
		}
	...
	struct With_Builder : Chunked_Array {
		using BASE = Chunked_Array;
		using BASE::BASE;
		// FORWARDING_CONSTRUCTOR(With_Builder, Chunked_Array) {}
...
template< class T >
using Chunked_Array = typename _::chunked_array::Context<
	T,
	false, // SPARSE
	false, // COUNT
	Memory_Block<T>
