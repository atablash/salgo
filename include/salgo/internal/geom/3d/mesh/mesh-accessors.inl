...
struct Verts_Context {
	...
	static constexpr auto Has_Data = P::Has_Vert_Data;
...
struct VertPolys_Context {
	...
	static constexpr auto Has_Data = P::Has_PolyVert_Data;
...
struct Polys_Context : private P {
	...
	static constexpr auto Has_Data = P::Has_Poly_Data;
	...
	public:
		...
		auto polyVert(int ith) {
			DCHECK_GE(ith, 0) << "out of bounds";
			DCHECK_LT(ith, 3) << "out of bounds";
			return P::PolyVerts_Context::template create_accessor<C>( &CONT, H_PolyVert(HANDLE, ith) );
		};

		auto polyVert(int ith) const {
			DCHECK_GE(ith, 0) << "out of bounds";
			DCHECK_LT(ith, 3) << "out of bounds";
			return P::PolyVerts_Context::template create_accessor<CONST>( &CONT, H_PolyVert(HANDLE, ith) );
		};
		...
		auto polyEdge(int ith) {
			DCHECK_GE(ith, 0) << "out of bounds";
			DCHECK_LT(ith, 3) << "out of bounds";
			return P::PolyEdges_Context::template create_accessor<C>( &CONT, H_PolyEdge(HANDLE, ith) );
		};

		auto polyEdge(int ith) const {
			DCHECK_GE(ith, 0) << "out of bounds";
			DCHECK_LT(ith, 3) << "out of bounds";
			return P::PolyEdges_Context::template create_accessor<CONST>( &CONT, H_PolyEdge(HANDLE, ith) );
		};
		...
		void erase(ARGS&&... _args) {
			...
			if constexpr(P::Has_Vert_Poly_Links) {
				for(int ith=0; ith < 3; ++ith) {
					...
					DCHECK( v.poly_links( H_PolyVert(HANDLE, ith) ).found() );
					v.poly_links( H_PolyVert(HANDLE, ith) ).erase();

					if(erase_isolated_verts && v.poly_links.empty()) {
...
struct PolyEdges_Context : private P {
	...
	static constexpr auto Has_Data = P::Has_PolyEdge_Data;
	...
	public:
		...
		auto prev()       {  _check(); return Accessor<C>    (&CONT, HANDLE.prev());  }
		auto prev() const {  _check(); return Accessor<CONST>(&CONT, HANDLE.prev());  }

		auto next()       {  _check(); return Accessor<C>    (&CONT, HANDLE.next());  }
		auto next() const {  _check(); return Accessor<CONST>(&CONT, HANDLE.next());  }
...
public:
	...
	auto empty() const {  return P::raw_vs(_mesh).empty();  }
...
public:
	...
	auto empty() const {  return P::raw_ps(_mesh).empty();  }
...
public:
	...
	bool empty() const { return P::raw(_mesh, _vert).poly_links.empty(); }
