...
namespace mesh {
	...
	struct Context {
		...
		struct PolyVert :
				Add_data<PolyVert_Data, Has_PolyVert_Data>,
		...
		struct PolyEdge :
			Add_data<PolyEdge_Data, Has_PolyEdge_Data>,
			Add_link<H_PolyEdge, Has_Edge_Links> {};


		struct Poly :
				Add_data<Poly_Data, Has_Poly_Data>,
				Add_edges<std::array<PolyEdge,3>, Has_PolyEdge_Data || Has_Edge_Links> {

			std::array<PolyVert,3> verts;
		...
		public:
			...
			auto vertPolys()       { return A_Vert_PolyVerts<C>    ( _c(), _h() ); }
			auto vertPolys() const { return A_Vert_PolyVerts<CONST>( _c(), _h() ); }
		...
		public:
			...
			auto polyVert(int ith) {
				DCHECK_GE(ith, 0) << "out of bounds";
				DCHECK_LT(ith, 3) << "out of bounds";
				return A_Poly_PolyVert<C>( &_c(), H_PolyVert(_h(), ith) );
			};

			auto polyVert(int ith) const {
				DCHECK_GE(ith, 0) << "out of bounds";
				DCHECK_LT(ith, 3) << "out of bounds";
				return A_Poly_PolyVert<CONST>( &_c(), H_PolyVert(_h(), ith) );
			};
			...
			auto polyEdge(int ith) {
				DCHECK_GE(ith, 0) << "out of bounds";
				DCHECK_LT(ith, 3) << "out of bounds";
				return A_Poly_PolyEdge<C>( &_c(), H_PolyEdge(_h(), ith) );
			};
			...
			auto polyVerts()       {  return A_Poly_PolyVerts<C>    (_c(), _h());  }
			auto polyVerts() const {  return A_Poly_PolyVerts<CONST>(_c(), _h());  }

			auto polyEdges()       {  return A_Poly_PolyEdges<C>    (_c(), _h());  }
			auto polyEdges() const {  return A_Poly_PolyEdges<CONST>(_c(), _h());  }
			...
			void erase(const Erase_Params& par = Erase_Params()) {
				...
				if constexpr(Has_Vert_Poly_Links) {
					for(int ith=0; ith < 3; ++ith) {
						...
						DCHECK( v.poly_links( H_PolyVert(_h(), ith) ).exists() );
						v.poly_links( H_PolyVert(_h(), ith) ).erase();
		...
		struct Vert_PolyVerts_Context {
			...
			static constexpr auto Has_Data = Has_PolyVert_Data;
			...
			public:
				...
				auto polyVert()       { return A_Poly_PolyVert<C>    (&CONT, _get_polyvert_handle()); }
				auto polyVert() const { return A_Poly_PolyVert<CONST>(&CONT, _get_polyvert_handle()); }
			...
			private:
				...
				void _increment() { MUT_HANDLE.b = CONT._vs[ HANDLE.a ].poly_links( HANDLE.b ).iterator().next(); }
				void _decrement() { MUT_HANDLE.b = CONT._vs[ HANDLE.a ].poly_links( HANDLE.b ).iterator().prev(); }
		...
		struct Polys_Context {
			...
			private:
				...
				void _increment() { MUT_HANDLE = CONT._ps( HANDLE ).iterator().next(); }
				void _decrement() { MUT_HANDLE = CONT._ps( HANDLE ).iterator().prev(); }
		...
		struct Poly_PolyVerts_Context {
			...
			public:
				...
				auto prev_polyEdge()       { return A_Poly_PolyEdge<C>    (&CONT, H_PolyEdge(HANDLE.poly, (HANDLE.ith+2)%3) ); }
				auto prev_polyEdge() const { return A_Poly_PolyEdge<CONST>(&CONT, H_PolyEdge(HANDLE.poly, (HANDLE.ith+2)%3) ); }

				auto next_polyEdge()       { return A_Poly_PolyEdge<C>    (&CONT, H_PolyEdge(HANDLE.poly, HANDLE.ith) ); }
				auto next_polyEdge() const { return A_Poly_PolyEdge<CONST>(&CONT, H_PolyEdge(HANDLE.poly, HANDLE.ith) ); }

				auto opposite_polyEdge()       { return A_Poly_PolyEdge<C>    (&CONT, H_PolyEdge(HANDLE.poly, (HANDLE.ith+1)%3) ); }
				auto opposite_polyEdge() const { return A_Poly_PolyEdge<CONST>(&CONT, H_PolyEdge(HANDLE.poly, (HANDLE.ith+1)%3) ); }
		...
		struct Poly_PolyEdges_Context {
			using Container = Mesh;
			using Handle = H_PolyEdge;
			static constexpr auto Has_Data = Has_PolyEdge_Data;
			...
			public:
				...
				auto prev_polyVert()       { _check_exists(); return A_Poly_PolyVert<C>    (&CONT, H_PolyVert(HANDLE.poly, HANDLE.ith) ); }
				auto prev_polyVert() const { _check_exists(); return A_Poly_PolyVert<CONST>(&CONT, H_PolyVert(HANDLE.poly, HANDLE.ith) ); }

				auto next_polyVert()       { _check_exists(); return A_Poly_PolyVert<C>    (&CONT, H_PolyVert(HANDLE.poly, (HANDLE.ith+1)%3) ); }
				auto next_polyVert() const { _check_exists(); return A_Poly_PolyVert<CONST>(&CONT, H_PolyVert(HANDLE.poly, (HANDLE.ith+1)%3) ); }

				auto opposite_polyVert()       { _check_exists(); return A_Poly_PolyVert<C>    (&CONT, H_PolyVert(HANDLE.poly, (HANDLE.ith+2)%3) ); }
				auto opposite_polyVert() const { _check_exists(); return A_Poly_PolyVert<CONST>(&CONT, H_PolyVert(HANDLE.poly, (HANDLE.ith+2)%3) ); }


				auto prev_vert()       { _check_exists(); return prev_polyVert().vert(); }
				auto prev_vert() const { _check_exists(); return prev_polyVert().vert(); }

				auto next_vert()       { _check_exists(); return next_polyVert().vert(); }
				auto next_vert() const { _check_exists(); return next_polyVert().vert(); }

				auto opposite_vert()       { _check_exists(); return opposite_polyVert().vert(); }
				auto opposite_vert() const { _check_exists(); return opposite_polyVert().vert(); }
				...
				auto linked_polyEdge() {
					static_assert(Has_Edge_Links, "linked_polyEdge() requires EDGE_LINKS");
					_check_exists();
			...
			private:
				...
				void _check_exists() const {
					DCHECK(exists()) << "non-existing polyEdge accessed";
				}
			}; // Accessor
		...
		public:
			...
			template<class... ARGS, class SFINAE = decltype(Vert(std::declval<ARGS>()...))>
			auto add(ARGS&&... args) {
				auto v = _mesh._vs.add( std::forward<ARGS>(args)... );
				return A_Vert<MUTAB>(&_mesh, v.handle());
			}
		...
		public:
			...
			auto add(IDX_Vert a, IDX_Vert b, IDX_Vert c) {
				...
				if constexpr(Has_Vert_Poly_Links) {
					_mesh._vs[ a ].poly_links.emplace( acc.polyVert(0) );
					_mesh._vs[ b ].poly_links.emplace( acc.polyVert(1) );
					_mesh._vs[ c ].poly_links.emplace( acc.polyVert(2) );
				}
		...
		public:
			...
			bool empty() const { return _mesh._vs[_vert].poly_links.empty(); }
		...
		public:
			auto begin()       {  return I_Poly_PolyVert<C>    (&_mesh, H_PolyVert(_poly, 0));  }
			auto begin() const {  return I_Poly_PolyVert<CONST>(&_mesh, H_PolyVert(_poly, 0));  }
		...
		public:
			auto begin()       {  return I_Poly_PolyEdge<C>    (&_mesh, H_PolyEdge(_poly, 0));  }
			auto begin() const {  return I_Poly_PolyEdge<CONST>(&_mesh, H_PolyEdge(_poly, 0));  }
		...
		public:
			...
			using Vector          = Context::Vector;

			using Vert_Data      = Context::Vert_Data;
			using Subvert_Data   = Context::Subvert_Data;
			using Poly_Data      = Context::Poly_Data;
			using PolyVert_Data = Context::PolyVert_Data;
			using PolyEdge_Data = Context::PolyEdge_Data;
			...
			using SH_Vert      = Context::SH_Vert;
			
			using  H_Poly      = Context:: H_Poly;
			using SH_Poly      = Context::SH_Poly;

			using  H_PolyVert  = Context:: H_PolyVert;
			using SH_PolyVert  = Context::SH_PolyVert;
			
			using  H_PolyEdge  = Context:: H_PolyEdge;
			using SH_PolyEdge  = Context::SH_PolyEdge;
			
			using  H_Subvert   = Context:: H_Subvert;
			using SH_Subvert   = Context::SH_Subvert;
			...
			static constexpr bool Has_PolyVert_Data = Context::Has_PolyVert_Data;
			static constexpr bool Has_PolyEdge_Data = Context::Has_PolyEdge_Data;
		...
		public:
			...
			auto operator()(H_PolyVert handle)       {  return A_Poly_PolyVert<MUTAB>(this, handle);  }
			auto operator()(H_PolyVert handle) const {  return A_Poly_PolyVert<CONST>(this, handle);  }

			auto& operator[](H_PolyVert handle)       {  return A_Poly_PolyVert<MUTAB>(this, handle).data();  }
			auto& operator[](H_PolyVert handle) const {  return A_Poly_PolyVert<CONST>(this, handle).data();  }


			auto operator()(H_PolyEdge handle)       {  return A_Poly_PolyEdge<MUTAB>(this, handle);  }
			auto operator()(H_PolyEdge handle) const {  return A_Poly_PolyEdge<CONST>(this, handle);  }
		...
		struct With_Builder : Mesh {
			...
			using POLY_DATA =
				typename Context<Scalar, Vert_Data, Subvert_Data,
					NEW_POLY_DATA, PolyVert_Data, PolyEdge_Data, Verts_Erasable_Mode, Polys_Erasable_Mode,
					Has_Edge_Links, Has_Vert_Poly_Links> :: With_Builder;

			template<class NEW_POLYVERT_DATA>
			using POLYVERT_DATA =
				typename Context<Scalar, Vert_Data, Subvert_Data,
					Poly_Data, NEW_POLYVERT_DATA, PolyEdge_Data, Verts_Erasable_Mode, Polys_Erasable_Mode,
					Has_Edge_Links, Has_Vert_Poly_Links> :: With_Builder;

			template<class NEW_POLYEDGE_DATA>
			using POLYEDGE_DATA =
				typename Context<Scalar, Vert_Data, Subvert_Data,
					Poly_Data, PolyVert_Data, NEW_POLYEDGE_DATA, Verts_Erasable_Mode, Polys_Erasable_Mode,
					Has_Edge_Links, Has_Vert_Poly_Links> :: With_Builder;
