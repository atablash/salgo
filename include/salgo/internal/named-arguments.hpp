...
namespace internal::named_arguments {
...
struct Named_Arguments {
	using TLIST = decltype( internal::named_arguments::create_args_list( std::declval<ARGS>()... ) );
	TLIST _args;

	constexpr Named_Arguments(ARGS&&... args) : _args(
		internal::named_arguments::create_args_list( std::forward<ARGS>(args)... )
	) {}

	template<class ARG>
	constexpr auto&& operator()(ARG arg) { return internal::named_arguments::get(arg, _args).get(); }

	template<class ARG, class DEF>
	constexpr auto&& operator()(ARG arg, DEF&& def) {
		namespace det = internal::named_arguments;
		if constexpr(has(arg)) return det::get(arg, _args).value;
		else return std::forward<DEF>(def);
	}

	template<class ARG>
	static constexpr bool has(ARG) { return internal::named_arguments::has<ARG, ARGS...>(); }
...
struct Named_Argument {
	...
	constexpr auto operator=(Named_Arguments<ARGS...>&& args) const {
		if constexpr(internal::named_arguments::has<Named_Argument,ARGS...>()) {
			return internal::named_arguments::get(*this, args._args).const_rvalue_reference();
		}
		else {
	...
	constexpr auto operator<<(Named_Arguments<ARGS...>&& args) const {
		if constexpr(internal::named_arguments::has<Named_Argument,ARGS...>()) {
			return internal::named_arguments::get(*this, args._args).rvalue_reference();
		}
		else {
			return Named_Argument_Value<void,void>();
		}
	...
	constexpr auto operator=(Named_Arguments<ARGS...>& args) const {
		if constexpr(internal::named_arguments::has<Named_Argument,ARGS...>()) {
			return internal::named_arguments::get(*this, args._args).const_lvalue_reference();
		}
		else {
			return Named_Argument_Value<void,void>();
		}
	...
	constexpr auto operator<<(Named_Arguments<ARGS...>& args) const {
		if constexpr(internal::named_arguments::has<Named_Argument,ARGS...>()) {
			return internal::named_arguments::get(*this, args._args).lvalue_reference();
		}
		else {
			return Named_Argument_Value<void,void>();
		}
